...
class ShapeMultiViewLoader(Sequence):
    ...
    def __getitem__(self, index):
        ...
        for model_name in batch_model_names:
            ...
            pose = np.array([
                model_rows.iloc[0]['pos_x'], 
                model_rows.iloc[0]['pos_y'],
                model_rows.iloc[0]['pos_z'],
                model_rows.iloc[0]['rot_x'],
                model_rows.iloc[0]['rot_y'],
                model_rows.iloc[0]['rot_z'],
                model_rows.iloc[0]['rot_w']
            ], dtype=np.float32)
                
                # Normalização (opcional)
            pose[:3] = (pose[:3] - config.POSITION_RANGE[0]) / (config.POSITION_RANGE[1] - config.POSITION_RANGE[0]) * 2 - 1
            ...
            batch_labels.append(sample["pose"])
            if self.include_aux:
