...
class CuboVirtualAR:
    ...
    def desenhar_realidade_aumentada(self, frame: np.ndarray):
        ...
        if len(self.marcadores_ref) == 2 and 0 in self.marcadores_ref and 1 in self.marcadores_ref:
            ...
            if self.sistema_calibrado:
                ...
                # Calcular e mostrar coordenadas dos grupos dentro do sistema de referência
                self._calcular_coordenadas_grupos(frame)
        else:
            # Se não temos os 2 marcadores, perder calibração
            self.sistema_calibrado = False
        
        # Apenas destacar marcadores dos grupos (sem cubos)
        self._destacar_marcadores_grupos(frame)
    ...
    def _calcular_coordenadas_grupos(self, frame: np.ndarray):
        """Calcula e mostra coordenadas dos grupos no sistema de referência."""
        if not self.sistema_calibrado:
            return
        
        print("\n=== COORDENADAS NO SISTEMA DE REFERÊNCIA ===")
        
        # Processar Grupo 1
        if self.marcadores_g1:
            print("GRUPO 1 (Vermelho):")
            for marker_id, info in self.marcadores_g1.items():
                coords = self._converter_para_sistema_referencia(info['posicao'])
                if coords:
                    print(f"  ID {marker_id}: X={coords[0]:.1f}mm, Y={coords[1]:.1f}mm, Z={coords[2]:.1f}mm")
    ...
    def _converter_para_sistema_referencia(self, posicao_mundo: np.ndarray) -> tuple:
        """Converte posição do mundo para sistema de coordenadas das referências."""
        if not self.sistema_calibrado:
            return None
        
        # Calcular posição relativa à origem (marcador 0)
        pos_relativa = posicao_mundo - self.origem_3d
        
        # Projetar nos eixos do sistema de referência
        x_ref = np.dot(pos_relativa, self.vetor_x) * 1000  # Converter para mm
        y_ref = np.dot(pos_relativa, self.vetor_y) * 1000  # Converter para mm
        z_ref = pos_relativa[2] * 1000  # Z é simplesmente a altura em mm
        
        return (x_ref, y_ref, z_ref)
    
    def _destacar_marcadores_grupos(self, frame: np.ndarray):
        """Destaca marcadores dos grupos com círculos (sem cubos)."""
        # Grupo 1 - Vermelho
        for marker_id, info in self.marcadores_g1.items():
            corners = info['corners']
            centro = np.mean(corners[0], axis=0).astype(int)
            cv2.circle(frame, tuple(centro), 25, (0, 0, 255), 3)
            cv2.putText(frame, f"G1({marker_id})", 
        ...
        for marker_id, info in self.marcadores_g2.items():
            corners = info['corners']
            centro = np.mean(corners[0], axis=0).astype(int)
            cv2.circle(frame, tuple(centro), 25, (255, 0, 0), 3)
            cv2.putText(frame, f"G2({marker_id})", 
                       (centro[0] + 30, centro[1]), 
    ...
    def imprimir_info_detalhada(self):
        ...
        if self.sistema_calibrado:
            ...
            print(f"  Sistema de coordenadas calibrado")
        else:
            print("CUBO PRINCIPAL: INATIVO (necessário IDs 0 e 1)")
        
        # Coordenadas dos grupos no sistema de referência
        if self.sistema_calibrado:
            print(f"\nCOORDENADAS NO SISTEMA DE REFERÊNCIA:")
            
            # Grupo 1
            print(f"GRUPO 1 (Vermelho): {len(self.marcadores_g1)} detectados")
            if self.marcadores_g1:
                for marker_id, info in self.marcadores_g1.items():
                    coords = self._converter_para_sistema_referencia(info['posicao'])
                    if coords:
                        print(f"  ID {marker_id}: X={coords[0]:.2f}mm, Y={coords[1]:.2f}mm, Z={coords[2]:.2f}mm")
            else:
                print("  Nenhum marcador do grupo 1 detectado")
            
            # Grupo 2
            print(f"GRUPO 2 (Azul): {len(self.marcadores_g2)} detectados")
            if self.marcadores_g2:
                for marker_id, info in self.marcadores_g2.items():
                    coords = self._converter_para_sistema_referencia(info['posicao'])
                    if coords:
                ...
                print("  Nenhum marcador do grupo 2 detectado")
        else:
            print("\nCOORDENADAS: Sistema não calibrado")
