...
class CuboVirtual:
    ...
    def _desenhar_cubo_virtual(self, frame: np.ndarray, rvec: np.ndarray, tvec: np.ndarray) -> None:
        """Desenha cubo virtual 3D sólido sobre o marcador."""
        # Definir vértices do cubo (5cm de lado), centrado no marcador
        cube_pts = np.array([
            [-0.025, -0.025, 0.025], [0.025, -0.025, 0.025], [0.025, 0.025, 0.025], [-0.025, 0.025, 0.025], # Face frontal
            [-0.025, -0.025, -0.025], [0.025, -0.025, -0.025], [0.025, 0.025, -0.025], [-0.025, 0.025, -0.025] # Face traseira
        ], dtype=np.float32)

        # Cores para cada face (BGR)
        cores = [
            (0, 255, 0), (0, 150, 0), (0, 200, 0), # Verde
            (0, 255, 0), (0, 150, 0), (0, 200, 0) # Verde
        ]
        ...
        # Ordenar as faces do cubo pela distância Z para renderizar corretamente
        # As faces mais próximas (menor Z) são desenhadas por último para sobreporem as mais distantes
        distancias_z = [np.mean([tvec[2] + cube_pts[i][2] for i in face]) for face in faces_indices]
        faces_ordenadas = sorted(zip(faces_indices, cores, distancias_z), key=lambda x: x[2], reverse=True)

        for face, cor, _ in faces_ordenadas:
            pontos_face = np.array([img_pts[i] for i in face])
            cv2.fillConvexPoly(frame, pontos_face, cor)
        
        # Desenhar as bordas para melhor visualização
