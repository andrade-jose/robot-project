...
class EnhancedMultiviewCNN:
    ...
    def build_multiview_model(img_size, num_classes, include_aux=False, 
        ...
        common_dim = 512
        x_rgb = Dense(common_dim, activation='relu')(x_rgb)
        x_depth = Dense(common_dim, activation='relu')(x_depth)
        ...
        x_combined = Add()([x_rgb_att, x_depth_att])
        x_combined = LayerNormalization()(x_combined)
        
        # Auxiliary features processing
        if include_aux:
            # Enhanced auxiliary processing
            x_aux = Dense(64, activation='relu')(aux_input)
            x_aux = LayerNormalization()(x_aux)
            x_aux = Dense(128, activation='relu')(x_aux)
            x_aux = Dropout(0.1)(x_aux)
            x_aux = Dense(common_dim, activation='relu')(x_aux)
            
            # Fuse auxiliary features
            x_combined = Add()([x_combined, x_aux])
            x_combined = LayerNormalization()(x_combined)
        ...
        x = Bidirectional(GRU(256, return_sequences=True, dropout=0.2, recurrent_dropout=0.1))(x)
        x = Bidirectional(GRU(128, dropout=0.2, recurrent_dropout=0.1))(x)
    ...
    def build_lightweight_model(img_size, num_classes, include_aux=False, 
        ...
        if include_aux:
            ...
            x_aux = RepeatVector(6)(x_aux)
            x = Concatenate(axis=-1)([x, x_aux])
