...
# Imports locais
from advanced_cnn import Multiview_CNN
from callbacks import get_training_callbacks
from loading_csv import ShapeMultiViewLoader
from preprocessing import MultiViewAugmentor
...
def parse_args():
    ...
    if parser.parse_args().optimizer.lower() not in VALID_OPTIMIZERS:
        parser.error(f"Otimizador inválido: {parser.parse_args().optimizer}. Escolha entre {VALID_OPTIMIZERS}")
